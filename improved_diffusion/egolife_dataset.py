...
class ContinuousEgoLifeDataset(Dataset):
    LATENT_FPS = 20
    USE_FP16 = True
    ...
    def _initialize_file_index_mapping(self):
        pt_files = self.dataset_path.rglob('batch_*.pt')
...
class ChunkedEgoLifeDataset(ContinuousEgoLifeDataset):
...
class SpacedEgoLifeDataset(ContinuousEgoLifeDataset):
...
if __name__ == "__main__":
    dataset_path = "/ubc/cs/research/plai-scratch/jason/continual-diffusion/datasets/egolife"
    output_video_folder = "/ubc/cs/research/plai-scratch/jason/continual-diffusion/datasets/egolife/test_decoded"
    use_fp16 = True
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    FINAL_FRAME_SIZE = (512, 512)

    # dataset = ChunkedEgoLifeDataset(dataset_path, window_length=1250)
    # dataset = SpacedEgoLifeDataset(16, dataset_path, window_length=1250)
    dataset = ChunkedEgoLifeDataset(dataset_path, window_length=100)
    dataset.set_test()
    dataloader = DataLoader(dataset, batch_size=1, num_workers=2) #, collate_fn=ContinuousEgoLifeDataset.collate_fn)
