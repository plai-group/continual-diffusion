...
# CFG variant: action_dropout_prob=0.1 so the model also learns an unconditional
# mode, then samples with guidance. The plain variant is train_vdt_action.sh;
# the ONLY differences between the two files are ACTION_DROPOUT_PROB and CFG_SCALE.
#
...
REPO=/ubc/cs/research/plai-scratch/ctardy/projects/continual-diffusion.worktrees/issue-59
...
VAL_OUT=$REPO/results/action_validation_cfg
...
RUN_NAME="59 - vdt_action_cond_cfg"
...
ACTION_DROPOUT_PROB=0.1    # 10% of samples train with the null action embedding,
                           # so the model learns an unconditional mode too.
                           # Dropped per SAMPLE (whole video), not per frame --
                           # CFG needs a consistent unconditional mode.
CFG_SCALE=3.0              # guidance weight at validation sampling.
