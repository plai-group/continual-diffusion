...
#
# Issue #58 — VDT on plaicraft-debug (plai-group/plaicraft-debug#58).
# Action-UNCONDITIONAL. 24x40 pixel space, patch 4 -> 60 tokens/frame, T=20
# (10 observed / 10 generated, repo-default autoregressive masking).
#
# No-arg launch from repo root:   sbatch ./scripts/train_vdt_debug.sh
...
# This repo (unlike plaicraft-model-pi0) does NOT gitignore scripts/*.sh, so
...
MASKING_MODE=autoregressive  # fixed first-half observed / second-half latent,
                             # frames in order. flexible/autoflex permute frames,
                             # which VDT's fixed time_embed cannot express.
...
echo "staged $(find "$DEBUG_TOY_ROOT" -name '*.hdf5' | wc -l) sessions"
