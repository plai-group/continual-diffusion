...
def test_load_or_build_dispatches_to_raw_keys_and_symlogs_mouse(tmp_path):
    """load_or_build's raw branch is the MODEL's conditioning encoding: keys match
    load_or_build_raw exactly, but mouse is symlog-compressed on top -- load_or_build_raw
    stays the un-symlogged ground truth for overlays/interventions."""
    session_dir = tmp_path / "sess"
    (session_dir / "encoded_video_hdf5").mkdir(parents=True)
    import h5py
    with h5py.File(session_dir / "encoded_video_hdf5" / "sess_encoded_video.hdf5", "w") as f:
