...
def test_validate_action_encoding_raw_fused_is_fps_gated_like_raw():
    """raw_fused is a layout change on raw, so it inherits raw's 12.5 fps gate: overlay
    windows and start_frame math are hardcoded at DECODE_VIDEO_FPS, and a 10 fps corpus
    would read the wrong ones silently rather than fail."""
...
def test_load_or_build_raw_fused_does_not_create_a_third_cache_file(tmp_path):
    """raw_fused is a view over raw's cache, so it must leave exactly raw's artifacts
    behind -- the two .npy arrays plus the binning-rule sidecar, nothing extra."""
    raw_dir = _make_session(tmp_path / "raw", n_ticks=2)
    da.load_or_build(raw_dir, action_encoding="raw")
    expected = sorted(p.name for p in raw_dir.glob("actions_*"))

    fused_dir = _make_session(tmp_path / "fused", n_ticks=2)
    da.load_or_build(fused_dir, action_encoding="raw_fused")
    assert sorted(p.name for p in fused_dir.glob("actions_*")) == expected
